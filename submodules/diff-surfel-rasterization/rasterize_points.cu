    ...
    const bool include_feature)
{
  ...
  return std::make_tuple(rendered, out_color, out_language_feature, out_others, radii, geomBuffer, binningBuffer, imgBuffer);
}
    ...
    const bool include_feature) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dlanguage_feature, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
